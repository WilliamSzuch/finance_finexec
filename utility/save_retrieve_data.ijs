...
'' savedataTemp y
:
TempKeyfile =. jpath '~user/temp/tempkeyfile.ijf'
if. 0 = # y do. TempKeyfile =.  else. TempKeyfile =. y end.
ferase TempKeyfile
keycreate TempKeyfile
TempKeyfile saveData y
smoutput <'*** Data saved to Keyfile =>';TempKetfile
)
...
NB.- Writes all nouns from the keyfile:
NB.- jpath '~user/temp/tempkeyfile.ijf' 
NB.- or other keyfile.
NB.-syntax:
NB.+([K])writedataTemp('')
...
NB.-example:
NB.+  writedataTemp('')
NB.+  ('~user/temp/tempkeyfile.ijf')writedataTemp('')
NB. ---------------------------------------------------------
retrieveDataTemp =: 3 : 0
'' writeDataTemp ''
:
DEFAULT =. jpath '~user/temp/tempkeyfile.ijf' 
if. 0 = # x do. FILE =. DEFAULT else. FILE =. y end.
FILE retrieveData ''
...
NB.+F = path to folder
NB.-example:
NB. +  filesListn(jpath '~addons/finance/fp')
NB. ---------------------------------------------------------
filesListn =: 3 : 0
vfm =. 3 : '}:(,|."1 [ 1,.-. *./\"1 |."1 y='' '')#,y,.LF'
dltLastBox =: ] {.~ 1 >. _1 + #
